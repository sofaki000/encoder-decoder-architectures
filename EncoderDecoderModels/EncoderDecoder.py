...
class EncoderDecoderNetwork(nn.Module):
    def __init__(self, input_size, emb_size, weight_size, answer_seq_len, hidden_size=512, is_GRU=True):
        ...
        self.linear = nn.Linear(in_features= hidden_size, out_features = self.input_size)
    ...
    def forward(self, input):
        ...
        embedded_input = self.emb(input) # (bs, L, embd_size)

        # Encoding
        encoder_states, hc = self.enc(embedded_input) # encoder_state: (bs, L, H)
        encoder_states = encoder_states.transpose(1, 0) # (L, bs, H)

        # Decoding states initialization
        decoder_input = (torch.zeros(batch_size, self.emb_size)) # (bs, embd_size)
        hidden = (torch.zeros([batch_size, self.hidden_size]))   # (bs, h)
        cell_state = encoder_states[-1]  # (bs, h)

        # Broadcast context to (num_steps, batch_size, num_hiddens)
        context = cell_state.repeat(self.emb_size, batch_size, self.hidden_size)
        embs_and_context = torch.cat((embedded_input, context), -1)
        outputs, state = self.dec(embs_and_context, encoder_states)
        outputs = self.linear(outputs).swapaxes(0, 1)
        probs_for_current_batch = F.log_softmax(outputs)

        probs = probs_for_current_batch
        # # Decoding
